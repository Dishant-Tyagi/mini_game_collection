# =====================================
# main.py — PyInstaller Safe Version
# =====================================
import os
import sys
import logging
...
class MiniGameCollectionApp(App):
    ...
    def build_game_hub(self):
        ...
        for game_id in available_games:
            ...
            # ✅ FIXED: use resource_path for icons inside each game folder
            icon_path = resource_path(f"games/{game_id}/icon.png")
            if not os.path.exists(icon_path):
                icon_path = resource_path("assets/default_icon.png")

            card = HoverCard(orientation="vertical", padding=15, spacing=10,
                             size_hint_y=None, height=320)
            card.add_widget(Image(source=icon_path, allow_stretch=True,
                                  keep_ratio=True, size_hint_y=None, height=160))
    ...
    def build_stats_screen(self):
        ...
        root = BoxLayout(orientation="vertical", spacing=20, padding=[40, 30, 40, 30])
        title = Label(text="[b]Recent Matches[/b]", markup=True, font_size=38,
        ...
        if not stats:
            root.add_widget(Label(text="No match data available.",
                                  color=(0.7, 0.7, 0.7, 1), font_size=20))
        else:
            ...
            for h in headers:
                table.add_widget(Label(text=f"[b]{h}[/b]", markup=True, font_size=20,
                                       color=(0.9, 0.9, 0.9, 1),
            ...
            for record in stats:
                ...
                for text in [game, result, duration]:
                    lbl = Label(text=str(text), color=(1, 1, 1, 1),
                                font_size=16, size_hint_x=None, width=200,
                                halign="center", valign="middle")
        ...
        back_btn = Button(text="Back to Menu", size_hint=(None, None), size=(220, 50),
                          background_normal="", background_color=(0.3, 0.5, 0.9, 1),
